 ...
 */
export const formatMoney = (cents: string | number, formatString = "${{amount}}"): string => {
  ...
  const amountWithCommaSeparator = amount.replace(/\./g, ",").replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.");

  let formattedString = formatString.replace(/\{\{\s*amount\s*\}\}/, amount);
  formattedString = formattedString.replace(/\{\{\s*amount_no_decimals\s*\}\}/, amountNoDecimals);
  formattedString = formattedString.replace(/\{\{\s*amount_with_comma_separator\s*\}\}/, amountWithCommaSeparator);
