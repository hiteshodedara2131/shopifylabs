...
searchInput.addEventListener('input', (e) => {
  handleSearch(e.target.value);
			...
			shopifyUseCase:
				"Debounce predictive search input to avoid excessive API calls.",
		},
		throttle: {
			title: "throttle(fn, limit)",
			description:
				"Creates a throttled version of a function that invokes at most once per limit milliseconds.",
...
window.addEventListener('scroll', handleScroll);`,
		...
		deepMerge: {
			...
			description:
				"Deeply merges two or more objects. Later sources override earlier ones. Arrays are replaced.",
...
const config = deepMerge(defaults, userSettings);`,
			parameters: [
				...
				},
				{
					name: "...sources",
					type: "Partial<T>[]",
					description: "One or more source objects to merge",
				},
			],
			returns: "T — The merged object",
			shopifyUseCase:
				"Merge default section settings with user overrides from section.settings.",
		},
		uniqueId: {
			title: "uniqueId(prefix?)",
			description:
				"Generates a unique string ID combining a sequential counter and timestamp.",
...
const id = uniqueId('cart'); // "cart_1_m3k2f9a"`,
			...
			shopifyUseCase:
				"Generate unique IDs for dynamically created cart line items or section blocks.",
		},
	},
	dom: {
		"query-one": {
...
const heading = $('h2', sectionElement);`,
		...
		"query-all": {
			...
			description:
				"Selects all elements matching the given CSS selector. Returns a real Array.",
...
cards.forEach(card => card.classList.add('active'));`,
		...
		on: {
			...
			description:
				"Attaches an event listener. Supports delegation via CSS selectors.",
...
on('submit', formElement, (e) => e.preventDefault());`,
			parameters: [
				{ name: "eventName", type: "string", description: "DOM event name" },
				{
					...
					description: "CSS selector or direct element",
				},
				{
					name: "handler",
					type: "(event: T) => void",
			...
			parameters: [
				{
					...
					description: "Single item or array",
				},
			],
...
export default async function FeaturePage({
	...
	params: Promise<{ package: string; feature: string }>;
}) {
						...
						<table className="w-full text-left text-sm border-collapse">
							...
							<tbody className="text-slate-400 divide-y divide-white/5">
								{doc.parameters.map((param) => (
									...
									>
										<td className="px-6 py-4 font-mono text-brand-teal">
											{param.name}
										</td>
