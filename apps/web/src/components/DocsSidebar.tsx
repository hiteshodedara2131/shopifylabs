...
import { FiChevronDown, FiChevronRight, FiBox, FiCode, FiZap, FiCommand, FiBook } from "react-icons/fi";

const PACKAGES = [
  { id: "utils", name: "utils", fullName: "@shopifylabs/utils", icon: FiZap, color: "text-brand-purple" },
  { id: "dom", name: "dom", fullName: "@shopifylabs/dom", icon: FiCode, color: "text-brand-blue" },
  { id: "ajax", name: "ajax", fullName: "@shopifylabs/ajax", icon: FiBox, color: "text-brand-teal" },
  { id: "cli", name: "cli", fullName: "@shopifylabs/cli", icon: FiCommand, color: "text-brand-pink" },
];

const DOC_STRUCTURE: Record<string, any> = {
  utils: [
...
export default function DocsSidebar() {
  ...
  const currentPackageId = PACKAGES.find(p => pathname.startsWith(`/docs/${p.id}`))?.id || "utils";
  const currentPackage = PACKAGES.find(p => p.id === currentPackageId)!;

  useEffect(() => {
    // Expand sections that contain the active link
    const sectionsToExpand: string[] = [];
    DOC_STRUCTURE[currentPackageId]?.forEach((section: any) => {
      if (section.items?.some((item: any) => item.href === pathname)) {
  ...
  return (
    <aside className={`flex flex-col border-r border-white/5 bg-[#070B14] transition-all duration-300 ${isOpen ? "w-64" : "w-16"} hidden md:flex h-[calc(100vh-4rem)] sticky top-16`}>
      {/* Package Selector */}
        ...
        {isOpen ? (
          <div className="relative group">
            <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none text-slate-400 group-focus-within:text-brand-teal transition-colors">
              <currentPackage.icon className={`w-4 h-4 ${currentPackage.color}`} />
            </div>
            <select
            ...
            >
              {PACKAGES.map(pkg => (
                <option key={pkg.id} value={pkg.id}>{pkg.fullName}</option>
              ))}
            </select>
          ...
          <div className="flex justify-center">
            <currentPackage.icon className={`w-6 h-6 ${currentPackage.color}`} />
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        {isOpen ? (
          <div className="space-y-6">
            {DOC_STRUCTURE[currentPackageId]?.map((section: any) => (
              <div key={section.title}>
                {section.items ? (
                  <div>
                    ...
                    >
                      {section.title}
                      {expandedSections.includes(section.title) ? <FiChevronDown /> : <FiChevronRight />}
                    </button>
                            ...
                            <li key={item.href}>
                              <Link
                                ...
                                className={`block py-1.5 text-sm transition-colors ${
                                  pathname === item.href ? "text-brand-teal font-medium" : "text-slate-400 hover:text-white"
                                }`}
                              >
                                {item.title}
                              </Link>
                  ...
                  <Link
                    ...
                    className={`block py-1 text-sm font-medium transition-colors ${
                      pathname === section.href ? "text-brand-teal" : "text-slate-400 hover:text-white"
                    }`}
                  >
                    {section.title}
                  </Link>
