...
export default function InstallTabs({ packageName, isGlobal = false }: InstallTabsProps) {
  ...
  const getCommand = () => {
    switch (activeTab) {
      case "pnpm":
        return isGlobal ? `pnpm add -g ${packageName}` : `pnpm add ${packageName}`;
      case "npm":
        return isGlobal ? `npm install -g ${packageName}` : `npm install ${packageName}`;
      case "yarn":
        return isGlobal ? `yarn global add ${packageName}` : `yarn add ${packageName}`;
      case "cdn":
